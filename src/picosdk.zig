...
pub fn getBoardConfig(board_name: []const u8, pico_platform: []const u8, stdio_type: StdioType, cwy43_arch: Cwy43ArchType) !Board {
...
pub fn addPicoApp(b: *std.Build, option: PicoAppOption) !*std.Build.Step {
    ...
    _ = &zig_lib_buf;
    const zig_lib_str = try std.fmt.bufPrint(&zig_lib_buf, "{s}.o", .{option.app_name});
    ...
    var board_def_buf: [64]u8 = undefined;
    _ = &board_def_buf;
    const board_def_str = try std.fmt.bufPrint(&board_def_buf, "-DPICO_BOARD={s}", .{board_name});
    var platform_def_buf: [64]u8 = undefined;
    _ = &platform_def_buf;
    const platform_def_str = try std.fmt.bufPrint(&platform_def_buf, "-DPICO_PLATFORM={s}", .{pico_platform});
    var proj_name_buf: [64]u8 = undefined;
    _ = &proj_name_buf;
    const proj_name_str = try std.fmt.bufPrint(&proj_name_buf, "-DPROJ_NAME={s}", .{option.app_name});
    ...
    const cmake_argv = [_][]const u8{
        "cmake", "-B", "./build", "-S .", proj_name_str, cmake_pico_sdk_path, uart_or_usb, board_def_str, platform_def_str,
    };
